...
(function () {
	...
	Runner.prototype = {
		 ...
		 */
		playIntro: function () {
			document.querySelector(".timer").classList.remove("game__hide");
			document.querySelector(".scorebar").classList.remove("game__hide");
			timeStart();
			// intro
		...
		onKeyDown: function (e) {
			if (e.target != this.detailsButton) {
				if (!this.crashed && (Runner.keycodes.JUMP[String(e.keyCode)] || e.type == Runner.events.TOUCHSTART)) {
					...
					if (!this.tRex.jumping) {
						this.tRex.startJump();
						document.querySelector(".jump__sound").play();
					}
				}
		...
		gameOver: function () {
			...
			if (!this.gameOverPanel) {
				...
				console.log("skor: " + Math.ceil(this.distanceRan));
				timePaused();
				setTimeout(function () {
					document.querySelector(".step__two").classList.add("game__hide");
					document.querySelector(".step__three").classList.remove("game__hide");
	...
	(Obstacle.MAX_OBSTACLE_LENGTH = 3),
		(Obstacle.prototype = {
			...
			init: function (speed) {
				...
				// console.log("position : " + this.dimensions.WIDTH);
				this.draw();
	...
	Trex.collisionBoxes = [
		...
		new CollisionBox(1, -1, 115, 36),
		new CollisionBox(32, 0, 93, 46),
		new CollisionBox(10, 35, 99, 38),
		new CollisionBox(1, 24, 114, 35),
		new CollisionBox(5, 30, 106, 34),
		new CollisionBox(9, 34, 100, 34),
	];
	/**
	 * Animation states.
	 * @enum {string}
	...
	DistanceMeter.prototype = {
		 ...
		 * @return {boolean} Whether the acheivement fx should be played.
		 */
		...
		setHighScore: function (distance) {
			...
			// print skor
			var skorAkhir = highScoreStr.replace(/^0+/, "");
			document.querySelector(".endtime").innerHTML = skorAkhir + " poin";
		},
		/**
...
var stepFour = document.querySelector(".step__four");
var btnStart = document.querySelector(".btn__start");
var btnSubmit = document.querySelector(".submitform");
...
btnStart.addEventListener("click", function () {
	stepOne.classList.add("game__hide");
	bgSound.play();
});
...
function getTime() {
	...
	if (splitTime[0] == "00") {
		// endtime.innerHTML = splitTime[1] + " detik";
	} else {
		// endtime.innerHTML = splitTime[0] + " menit " + splitTime[1] + " detik";
	}
}
