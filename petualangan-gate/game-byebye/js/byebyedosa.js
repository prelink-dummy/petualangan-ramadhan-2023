...
window.onload = function () {
	...
	var level = {
		...
		radius: 14, // Bubble collision radius
	...
	function stateShootBubble(dt) {
		...
		for (var i = 0; i < level.columns; i++) {
			for (var j = 0; j < level.rows; j++) {
				...
				if (circleIntersection(player.bubble.x + level.tilewidth / 2, player.bubble.y + level.tileheight / 2, level.radius, coord.tilex + level.tilewidth / 2, coord.tiley + level.tileheight / 2, level.radius)) {
		...
		// Play shoot audio
		document.querySelector(".shoot__sound").play();
	}
		...
		if (animationstate == 1) {
			...
			if (!tilesleft) {
				...
				if (tilefound) {
					...
					setTimeout(function () {
						document.querySelector(".step__two").classList.add("steps__hide");
						document.querySelector(".step__three").classList.remove("steps__hide");
						// document.querySelector(".score").innerHTML = score;
						// document.querySelector(".bg__sound").pause();
					}, 2000);
	...
	function render() {
		...
		context.roundRect(level.x + 2, level.y + level.height + 18 - yoffset, level.width - 10, 1.1 * level.tileheight, 40);
		context.fill();

		// Draw score
		context.fillStyle = "#ffffff";
		context.font = "14px Roboto";
		var scorex = level.x + level.width - 120;
		var scorey = level.y + level.height + level.tileheight - yoffset - 5;
		drawCenterText("Score:", scorex, scorey, 150);
		context.font = "16px Arial";
		drawCenterText(score, scorex, scorey + 20, 150);
		...
		if (gamestate == gamestates.gameover) {
			...
			context.fillRect(level.x - 7, level.y - 4, level.width + 14, level.height + 4 * level.tileheight + 8 - yoffset);
	...
	function renderPlayer() {
		...
		// Draw the angle
		context.lineWidth = 5;
		context.strokeStyle = "#FFD7B3";
		context.beginPath();
		context.moveTo(centerx, centery);
		context.lineTo(centerx + 1 * level.tilewidth * Math.cos(degToRad(player.angle)), centery - 1 * level.tileheight * Math.sin(degToRad(player.angle)));
		context.stroke();
	...
	var btnReplay = document.querySelector(".btn__replay");
	var stepOne = document.querySelector(".step__one");
	var stepTwo = document.querySelector(".step__two");
	var stepThree = document.querySelector(".step__three");
	var bgSound = document.querySelector(".bg__sound");
	var watch = document.querySelector(".stopwatch");
	var scoreSpan = document.querySelector(".score");

	btnStart.addEventListener("click", function () {
		stepOne.classList.add("steps__hide");
		stepTwo.classList.remove("steps__hide");
		bgSound.play();
	});
	...
	function gameStartTime() {
		startGame = setTimeout(function () {
			setGameState(gamestates.gameover);
			setTimeout(function () {
				stepTwo.classList.add("steps__hide");
				stepThree.classList.remove("steps__hide");
...
let btnTnc = document.querySelector(".btn__tnc");
...
let bgmImg = bgm.querySelector("img");
let mus = true;
let bgSound = document.querySelector(".bg__sound");
