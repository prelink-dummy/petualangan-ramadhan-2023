	...
	for (let i = 0; i < anchors.length; i++) {
		anchors[i].addEventListener("click", (e) => {
			...
			if (href !== "#top") {
				...
				const position = window.pageYOffset + target.getBoundingClientRect().top - head;
	...
	window.addEventListener("DOMContentLoaded", (event) => {
		if (urlHash) {
			...
			setTimeout(function () {
				...
				const urlPosition = window.pageYOffset + urlTarget.getBoundingClientRect().top - head;
				window.scroll({
					top: urlPosition,
					behavior: "smooth",
				});
...
$(document).ready(function () {
	...
	if (!!e) {
		...
		$(".proses").click(function () {
			...
			document.querySelector(".ecard-img").style.backgroundImage = "url('" + imageData + "')";
